...
bool c_gui::Buttonbox(const char* label, bool selected) {
	...
	draw->AddRectFilled(bb.Min, bb.Max, gui.frame_active.to_im_color(0.8f * value->second), 4, 5);
...
void c_gui::StyleColorsDark(ImGuiStyle* dst) {
	style = dst ? dst : &ImGui::GetStyle();
	colors = style->Colors;
}
...
void c_gui::group_box(const char* name, ImVec2 size_arg) {
   ...
   // auto pos = window->DC.CursorPos;
	...
	BeginChild(std::string(name).append("").c_str(), size_arg, true);
	ImGui::SetCursorPos(ImVec2(0, 50));
	ImGui::BeginChild("##", { size_arg.x,size_arg.y }, false);
	ImGui::SetCursorPos(ImVec2(size_arg.x, 0));
   ...
   //  GetWindowDrawList( )->AddRect( pos + ImVec2( 0, 20 ), pos + size_arg, gui.border.to_im_color( ), 6 );
	...
	// GetWindowDrawList( )->AddText( pos + ImVec2( 12, 0 ), NULL, name );

   //  SetCursorPos( ImVec2( 12, 42 ) );
  //   PushStyleVar( ImGuiStyleVar_WindowPadding, { 0, 10 } );
	// BeginChild("##", size_arg, false);
