...
void CleanupRenderTarget()
{
	if (ImGuiMenu::mainRenderTargetView) { ImGuiMenu::mainRenderTargetView->Release(); ImGuiMenu::mainRenderTargetView = nullptr; }
}
void CreateRenderTarget()
{
	ID3D11Texture2D* pBackBuffer;
	ImGuiMenu::g_pSwapChain->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));
	ImGuiMenu::g_pd3dDevice->CreateRenderTargetView(pBackBuffer, nullptr, &ImGuiMenu::mainRenderTargetView);
	pBackBuffer->Release();
}