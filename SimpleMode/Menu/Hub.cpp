...
void Hub::Renderslidermodsettings(IModule* mod) {
	...
	for (auto& stg : mod->getSettings()) {
		...
		case ValueType::DOUBLE_T:
			//ImGui::SliderDouble(stg->name, &stg->value->_double, stg->minValue->_double, stg->maxValue->_double, "%.1f");
			break;
		case ValueType::INT_T:
			ImGui::SliderInt(stg->name, &stg->value->_int, stg->minValue->_int, stg->maxValue->_int);
			break;
		case ValueType::FLOAT_T:
			ImGui::SliderFloat(stg->name, &stg->value->_float, stg->minValue->_float, stg->maxValue->_float, "%.1f");
			break;
		case ValueType::ENUM_T:
			if (setting->minValue->_bool) {
				...
				for (auto it = enumData->Entrys.begin(); it != enumData->Entrys.end(); it++, e++) {
					EnumEntry i = *it._Ptr;
					ImGui::Checkbox(i.GetName().c_str(), &stg->value->_bool);
				}
			}
			break;
...
void Hub::renderCategory(Category category) {
	...
	for (auto& mod : moduleList) {
		...
		if (ImGui::Begin("##toggleguide", NULL,
			...
			ImGui::SetWindowPos(ImVec2(8, 20), ImGuiCond_Once);
			ImGui::SetWindowSize(ImVec2(300, ImGui::GetIO().DisplaySize.y), ImGuiCond_Once);
			if (mod->isEnabled()) {
				ImGui::PushFont(font);
				ImGui::TextColored(colors[ImGuiCol_Text], mod->getModuleName());
				ImGui::PopFont();
			}
			ImGui::End();
		}
